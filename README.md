...
- `app/src/main/java/com/nexuzy/publisher/workflow/` → fetch/verify/write/save/push orchestration.
...
## End-to-end button workflow (Fetch News)

When user taps **Fetch News** the expected pipeline is:
1. Read active RSS links.
2. Fetch RSS items with image detection/scraping.
3. Filter likely-news items.
4. Verify facts + confidence score using OpenAI.
5. Write/rewrite article + title using Gemini.
6. Clean grammar with Sarvam.
7. Save article in local Room DB as `draft`.
8. Push same article to WordPress as `draft` under article category.

This flow is implemented in `NewsWorkflowManager.fetchVerifyWriteSaveAndPushDraft(...)`.
